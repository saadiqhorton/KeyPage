...
- Legacy-vault migration: if `auth_stored_key` is still null, `GET /status` reports `proofReady: false` and the UI requires an unused recovery code. Recovery unwraps and re-encrypts in the browser, stores proof verifiers, and never transmits `authKeyB64`.
...
- Key Entry write proofs: create, import, update, and delete require a short-lived single-use challenge proof made with the browser-only auth key. The proof is bound to the challenge, HTTP method, API path, and SHA-256 digest of the effective JSON body, so a session cookie alone cannot mutate ciphertext and captured proofs cannot be replayed or moved to another payload. The auth proof key is retained only in the in-memory session-key store and cleared with the encryption key; read and reveal/copy activity paths remain session-only because they do not replace or destroy ciphertext
...
- First-boot claim: `POST /setup` requires a server-minted setup token, written to `<dataDir>/setup-token` (`0600`) on the first boot of an unclaimed vault. The plaintext token is not printed to stdout or container logs; operators read the file on the host (or via `docker compose exec`). Because the default bind is `0.0.0.0` and Cloudflare Tunnel arrives over loopback, no network-position gate can distinguish the owner; possession of the token is proof of host access. The token is deleted on successful setup (SAA-174, SAA-217). The setup `POST` body still rides the same connection: plain LAN HTTP exposes that body to observers. Docs warn operators to prefer Cloudflare Tunnel or a TLS reverse proxy before claiming. `KEYPAGE_REQUIRE_HTTPS_SETUP` defaults to `true`: setup is rejected unless Fastify sees HTTPS (`request.protocol === "https"`, which honors `X-Forwarded-Proto` only from peers in the `KEYPAGE_TRUSTED_PROXIES` IP/CIDR allowlist). `KEYPAGE_ALLOW_INSECURE_LOCAL_SETUP=true` permits one cleartext recovery claim from a client whose TCP socket is directly loopback; proxy-derived identities never satisfy it. There is no setup-token probe endpoint.
...
- All four recovery-code mint paths (first-time setup, account recovery, Master Password change, and regeneration) park freshly issued codes in vault wizard state inside VaultProvider; every route redirects to the dedicated `/recovery-codes` screen until the user acknowledges them, so a lock, a back navigation, or a remount cannot drop the only copy
- While a mint is in flight (`issuingRecoveryCodes`) or while a freshly issued recovery-code set is on screen, LoadingGate applies one shared `recoveryCodesExposurePending` predicate for beforeunload protection
...
- Recovery codes generated during setup, parked in wizard state, and shown on `/recovery-codes` until acknowledged (step 2 of the setup wizard lives on that screen)
...
- Listens on port **9090** (`PORT` env var; TypeScript default is `DEFAULT_LISTEN_PORT`)
